let setMealCart = document.getElementById("setmealCart");
...
let generateSetCartItems = () => {
    if(setMealItemsData.length !==0 && cart.length !== 0) {
        return (setMealCart.innerHTML = setMealItemsData
            .map((x) => {
                console.log(x);
                let { id, item } = x;
                let search = setMealItemsData.find((y)=>y.id === id) || [];
                    ...
                    <div class="cart-item" >
                        ...
                        <div class="details">
                            ...
                            <div class="cart-buttons">
                                <button onclick="setDecrement(${id})" class="decrease-item">-</button>
                                <span id=${id}>${item}</span>
                                <button onclick="setIncrement(${id})" class="increase-item">+</button>
                            </div>
...
generateSetCartItems();

let setIncrement = (id) => {
    let selectedItem = id;
    let search = setMealItemsData.find((x)=> x.id=== selectedItem.id);
    ...
    localStorage.setItem("setData", JSON.stringify(setMealItemsData));
};
...
let setDecrement = (id) => {
    ...
    update(selectedItem.id);
    setMealItemsData = setMealItemsData.filter ((x) => x.item !==0);
    generateSetCartItems();
    localStorage.setItem("data", JSON.stringify(cart));
    localStorage.setItem("setData", JSON.stringify(setMealItemsData));
}
